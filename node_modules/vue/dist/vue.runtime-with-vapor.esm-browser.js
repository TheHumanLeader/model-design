...
function isReusableAnchorCandidate(node, frag) {
	return !!node && (isComment(node, "") || isComment(node, "]") || frag !== void 0 && frag.anchorLabel !== void 0 && isComment(node, frag.anchorLabel));
}
...
function executeAnchorPlan(frag, plan) {
	...
	const createRuntimeAnchor = () => {
		...
		return frag.anchor = claimUntrackedAnchor(/* @__PURE__ */ createComment((_frag$anchorLabel = frag.anchorLabel) !== null && _frag$anchorLabel !== void 0 ? _frag$anchorLabel : ""));
	};
	try {
		switch (plan.kind) {
			case "reuse":
...
function createSlot(name = "default", rawProps, fallback, flags = 0) {
	...
	if (interopSlotsSource) {
		...
		if (slotFragment) fragment = slotFragment;
			...
			dynamicFragment = new DynamicFragment(132, "slot", false, false, false, void 0, _insertionAnchor);
...
var SlotFragment = class extends DynamicFragment {
	constructor(notifyParentBoundary = false, sharedFallback = false, inheritFallback = false, adoptAnchor) {
		super(132, "slot", false, false, false, void 0, adoptAnchor);
			...
			if (isHydrating$1) {
				if (this.sharedFallback || this.inheritFallback && !fallback) {
					...
					if (exposedValid) {
						...
						const anchor = claimUntrackedAnchor(/* @__PURE__ */ createComment((_this$anchorLabel = this.anchorLabel) !== null && _this$anchorLabel !== void 0 ? _this$anchorLabel : ""));
						this.anchor = anchor;
						...
						const anchor = this.anchor = claimUntrackedAnchor(/* @__PURE__ */ createComment((_this$anchorLabel2 = this.anchorLabel) !== null && _this$anchorLabel2 !== void 0 ? _this$anchorLabel2 : ""));
						const slotEnd = getCurrentSlotEndAnchor();
...
function resolveFragmentAnchor(adopt, anchorLabel) {
	...
	return claimUntrackedAnchor(anchorLabel ? /* @__PURE__ */ createComment(anchorLabel) : /* @__PURE__ */ createTextNode());
}
/**
...
function createPlainElement(comp, rawProps, rawSlots, isSingleRoot, once) {
	...
	if (rawSlots) {
		...
		if (isHydrating$1) {
			...
			if (rawSlots.$ && !child) child = el.appendChild(claimAnchor(/* @__PURE__ */ createComment("")));
			setCurrentHydrationNode(child);
		}
		if (rawSlots.$) {
			const frag = new DynamicFragment(512, isHydrating$1 ? "" : "slot");
...
function applyComponentFallthrough(instance) {
	if (instance.hasFallthrough && instance.type.inheritAttrs !== false) applyFallthroughAttrs(instance.block, instance, instance.renderScope);
}
...
function defineVaporAsyncComponent(source) {
	...
	return /* @__PURE__ */ defineVaporComponent({
		...
		setup() {
			const instance = currentInstance;
			markAsyncBoundary(instance);
			const frag = new DynamicFragment(0, "async component");
			let resolvedComp = getResolvedComp();
			if (resolvedComp) {
...
const VaporTransition = /*@__PURE__*/ decorate$1((props, { slots, expose }) => {
	...
	if (instance.rawSlots.$) {
		const frag = new DynamicFragment(0, "transition");
		let isMounted = false;
		renderEffect(() => {
...
function createIf(condition, b1, b2, flags = 1) {
	...
	if (flags & 16) {
		...
		frag = ok ? b1() : b2 ? b2() : [claimUntrackedAnchor(/* @__PURE__ */ createComment("if"))];
	} else {
		const index = flags >> 8;
		const keyed = index > 0;
		const keyBase = keyed ? (index - 1) * 2 : 0;
		const trackSlotBoundary = !!(flags & 128);
		const dynamicFragment = new DynamicFragment(256, "if", keyed, false, trackSlotBoundary, trackSlotBoundary ? () => {
...
function createKeyedFragment(key, render, trackSlotBoundary = false) {
	...
	const frag = new DynamicFragment(0, "keyed", true, true, trackSlotBoundary, trackSlotBoundary ? () => {
		...
		try {
			if (emptyLocalRange && newLength) {
				...
				if (resolvedAnchor) {
					...
					parentAnchor = claimUntrackedAnchor(/* @__PURE__ */ createComment("for"));
...
function createDynamicComponent(getter, rawProps, rawSlots, flags = 0) {
	...
	if (!isHydrating$1) resetInsertionState();
	const hydrationCursor = isHydrating$1 ? captureHydrationCursor() : null;
	const frag = new DynamicFragment(0, "dynamic-component", false, true, slotRoot, slotRoot ? () => {
...
function ensureForHydrationAnchorResolver() {
	...
	setForHydrationAnchorResolver((hydrationStart, anchorNode) => {
		...
		const parentAnchor = claimAnchor(/* @__PURE__ */ createComment("for"));
...
const VaporTransitionGroup = /*@__PURE__*/ decorate(/* @__PURE__ */ defineVaporComponent({
	...
	setup(props, { slots, expose }) {
		...
		const frag = new DynamicFragment(0, "transition-group");
		let currentTag;
		let currentSlot;
		let isMounted = false;
		renderEffect(() => {
	...
	for (let i = 0; i < children.length; i++) {
		...
		if (isValidTransitionBlock(child)) {
			if (child.$key != null) {
				...
				child.$transition = resolveTransitionHooks$1(child, props, state, instance);
