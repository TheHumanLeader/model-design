/**
  * @vue/runtime-vapor v3.6.0-rc.6
  * (c) 2018-present Yuxi (Evan) You and Vue contributors
  * @license MIT
  **/
import { EffectScope, Fragment, NULL_DYNAMIC_COMPONENT, Static, TransitionPropsValidators, VaporSlot, VueElementBase, activate, baseApplyTranslation, baseEmit, baseNormalizePropsOptions, baseResolveTransitionHooks, baseUseCssVars, callPendingCbs, callWithAsyncErrorHandling, callWithErrorHandling, checkTransitionMode, cloneVNode, createAppAPI, createAsyncComponentContext, createCanSetSetupRefChecker, createCommentVNode, createInternalObject, createVNode, currentInstance, currentInstance as currentInstance$1, deactivate, devtoolsComponentAdded, endMeasure, ensureHydrationRenderer, ensureRenderer, ensureValidVNode, ensureVaporSlotFallback, expose, filterModelListeners, flushOnAppMount, forceReflow, getAttributeMismatch, getComponentName, getContainerType, getFunctionalFallthrough, getInheritedScopeIds, getTransitionRawChildren, handleError, handleMovedChildren, hasCSSTransform, initFeatureFlags, invalidateMount, invokeDirectiveHook, isAsyncWrapper, isEmitListener, isFunctionalFallthroughKey, isHydrating, isHydratingEnabled, isKeepAlive, isMapEqual, isMismatchAllowed, isSetEqual, isTeleportDeferred, isTeleportDisabled, isTemplateNode, isTemplateRefKey, isVNode, isValidHtmlOrSvgAttribute, knownTemplateRefs, leaveCbKey, logMismatchError, markAsyncBoundary, matches, mergeProps, nextUid, normalizeContainer, normalizeRef, normalizeVNode, onBeforeMount, onBeforeUnmount, onBeforeUpdate, onMounted, onScopeDispose, onUpdated, parseEventName, patchStyle, performAsyncHydrate, performTransitionEnter, performTransitionLeave, popWarningContext, prepareTransitionLeave, prepareTransitionSwitch, pushWarningContext, queueJob, queuePostFlushCb, queuePostRenderEffect, rawVaporSlotKey, registerHMR, renderSlot, resetShapeFlag, resolveComponent, resolveDynamicComponent, resolvePropValue, resolveTeleportTarget, resolveTransitionChild, resolveTransitionProps, restoreCurrentInstance, setCurrentInstance, setCurrentRenderingInstance, setDevtoolsHook, setRef, setTransitionHooks, setVarsOnNode, shallowReactive, shallowRef, shouldSetAsProp, shouldSetAsPropForVueCE, shouldUpdateComponent, simpleSetCurrentInstance, startMeasure, toClassSet, toStyleMap, unregisterHMR, unsafeToTrustedHTML, useAsyncComponentState, useTransitionState, vModelCheckboxInit, vModelCheckboxUpdate, vModelGetValue, vModelSelectInit, vModelSetSelected, vModelTextInit, vModelTextUpdate, vShowHidden, vShowOriginalDisplay, validateProps, vdomSlotFallbackKey, warn, warnExtraneousAttributes, warnPropMismatch, watch, withAsyncContext as withAsyncContext$1, withCtx, withKeys, withModifiers, xlinkNS } from "@vue/runtime-dom";
import { EMPTY_ARR, EMPTY_OBJ, NO, NOOP, camelize, canSetValueDirectly, extend, extend as extend$1, getEscapedCssVarName, getGlobalThis, getSequence, hasOwn, includeBooleanAttr, invokeArrayFns, isArray, isFunction, isModelListener, isObject, isOn, isPlainObject, isPromise, isReservedProp, isSpecialBooleanAttr, isString, isSymbol, looseEqual, normalizeClass, normalizeCssVarValue, normalizeStyle, parseStringStyle, remove as remove$1, stringifyStyle, toDisplayString } from "@vue/shared";
import { EffectScope as EffectScope$1, ReactiveEffect, computed, effectScope, getCurrentScope, isReactive, isReadonly, isRef, isShallow, markRaw, onEffectCleanup, onScopeDispose as onScopeDispose$1, pauseTracking, proxyRefs, resetTracking, setActiveSub, setCurrentScope, shallowReactive as shallowReactive$1, shallowReadArray, shallowRef as shallowRef$1, toRaw, toReactive, toReadonly, traverse, unref, watch as watch$1 } from "@vue/reactivity";
...
let mismatchStampHook = null;
function setMismatchStampHook(hook) {
...
function setClass(el, value, isSVG = false, isNormalized = false) {
	...
	if (el.$root) setClassIncremental(el, value, isNormalized);
		...
		if (value !== el.$cls) {
			if (isSVG) el.setAttribute("class", el.$cls = value);
...
function isReusableAnchorCandidate(node, frag) {
	return !!node && (isComment(node, "") || isComment(node, "]") || !!(process.env.NODE_ENV !== "production") && frag !== void 0 && frag.anchorLabel !== void 0 && isComment(node, frag.anchorLabel));
}
...
function planReuseBoundaryClose(frag) {
	const slotAnchor = frag.__vf & 4 ? getCurrentSlotEndAnchor() : null;
	if (slotAnchor || !!(frag.__vf & 256) && isArray(frag.nodes) && frag.nodes.length > 1) {
...
function resolveDynamicAnchor(frag, isEmpty) {
	return planPendingSlotDecision(frag, isEmpty) || planReuseInjectedAnchor(frag) || (isEmpty ? planEmptyBranch(frag) : void 0) || planRestartFromRuntimeComment(frag) || planReuseBoundaryClose(frag) || planFromBlockBoundary(frag);
}
...
let t;
let templateCloneHook = null;
function setTemplateCloneHook(hook) {
...
function template(html, flags = 0, ns) {
	...
	return () => {
		...
		if (node) {
			const ret = templateCloneHook ? templateCloneHook(node) : node.cloneNode(true);
...
function stampSlotScopeIds(node) {
	if (node.nodeType === 1) setElementScopeIdsDeep(node, currentSlotScopeIds);
}
/**
...
function cloneStampedTemplate(prototype) {
...
function createSlot(name = "default", rawProps, fallback, flags = 0) {
	...
	if (interopSlotsSource) {
		...
		try {
			fragment = instance.appContext.vdom.slot(interopSlotsSource, name, slotProps, instance, {
				fallback,
				once,
				slotRoot,
				sharedFallback,
		...
		if (slotFragment) fragment = slotFragment;
			...
			dynamicFragment = new DynamicFragment(132, !!(process.env.NODE_ENV !== "production") ? "slot" : void 0, false, false, false, void 0, _insertionAnchor);
			fragment = dynamicFragment;
		}
		fragment.slotScopeIds = slotScopeIds;
		...
		const getBoundSlot = (slot) => {
			if (slot !== cachedSlot) {
				cachedSlot = slot;
				cachedBoundSlot = () => renderWithSlotScopeIds(slotScopeIds, () => once ? withOnceSlot(() => slot(slotProps)) : slot(slotProps));
			}
...
var RenderContextFragment = class extends VaporFragment {
	constructor(nodes, flags = 1) {
		...
		if (isSuspenseEnabled) this.renderSuspense = parentSuspense;
	}
...
function runWithFragmentCtxOnly(fragment, fn) {
	const suspense = isSuspenseEnabled ? fragment.renderSuspense || null : null;
	const restoreSuspense = isSuspenseEnabled && parentSuspense !== suspense;
	if (!restoreSuspense && currentSlotOwner === fragment.slotOwner && currentSlotBoundary === fragment.slotBoundary && currentSlotScopeIds === fragment.slotScopeIds) return fn();
...
var DynamicFragment = class extends RenderContextFragment {
	constructor(flags = 0, anchorLabel, keyed = false, locate = true, trackSlotBoundary = false, onInvalid, adoptAnchor) {
		...
		} else this.anchor = resolveFragmentAnchor(adoptAnchor, anchorLabel);
		if (trackSlotBoundary) trackSlotBoundaryDirtying(this, onInvalid);
	}
	get autoHydrate() {
		return true;
	...
	update(render, key = render, noScope = false) {
		...
		if (wasMounted) {
			...
			if (scope) {
				if (this.keepAliveCtx) deferRemoval = this.keepAliveCtx.prepareBranchRemoval(this, scope);
				else scope.stop();
			}
...
var SlotFragment = class extends DynamicFragment {
	constructor(notifyParentBoundary = false, sharedFallback = false, inheritFallback = false, adoptAnchor) {
		super(132, !!(process.env.NODE_ENV !== "production") ? "slot" : void 0, false, false, false, void 0, adoptAnchor);
	...
	get boundary() {
		return this.ownBoundary || (this.ownBoundary = createSlotBoundary(this, this.inheritFallback ? this.slotBoundary : null, () => this.localFallback, (force) => markSlotResolutionDirty(this, force), this.onContentInvalid));
	}
	...
	updateHydratingContent(render, key) {
		const contentStart = currentHydrationNode;
		const pending = startPendingSlotContentGuard(this.sharedFallback || hasSlotFallback(this.boundary), contentStart);
		try {
	...
	updateSlot(render, fallback, key = render || fallback) {
		...
		const slotRender = render ? () => withSlotBoundary(boundary, render) : () => EMPTY_BLOCK;
			...
			if (isHydrating$1) {
				if (this.sharedFallback || this.inheritFallback && !fallback) {
					...
					if (exposedValid) {
						...
						const anchor = claimUntrackedAnchor(!!(process.env.NODE_ENV !== "production") ? /* @__PURE__ */ createComment((_this$anchorLabel = this.anchorLabel) !== null && _this$anchorLabel !== void 0 ? _this$anchorLabel : "") : /* @__PURE__ */ createTextNode());
						this.anchor = anchor;
						claimPrecedingFragmentClose(slotEnd);
						const attachContent = createDeferredSlotAttach(contentStart, slotEnd, anchor, candidate, (candidate) => this.anchor = claimAnchor(candidate), () => this.nodes);
						...
						const anchor = this.anchor = claimUntrackedAnchor(!!(process.env.NODE_ENV !== "production") ? /* @__PURE__ */ createComment((_this$anchorLabel2 = this.anchorLabel) !== null && _this$anchorLabel2 !== void 0 ? _this$anchorLabel2 : "") : /* @__PURE__ */ createTextNode());
						const slotEnd = getCurrentSlotEndAnchor();
...
function move(block, parent, anchor = null, moveType = 1, parentComponent, parentSuspense) {
	if (block instanceof Node) {
		if (isTransitionEnabled && block instanceof Element && block.$transition && !block.$transition.disabled && moveType !== 2) {
...
function createComponent(component, rawProps, rawSlots, isSingleRoot, once, appContext = currentInstance && currentInstance.appContext || emptyContext, managedMount = false, ce) {
	...
	if (isHydrating$1) {
		...
		if (component.__multiRoot && isComment(currentHydrationNode, "[")) {
			...
			exitHydrationBoundary = enterHydrationBoundary(hydrationClose && claimAnchor(hydrationClose));
			setCurrentHydrationNode(currentHydrationNode.nextSibling);
		}
	...
	try {
		...
		if ((isSingleRoot || (isTransitionEnabled ? currentInstance && isVaporTransition(currentInstance.type) : false)) && isVaporComponent(currentInstance) && currentInstance.type.inheritAttrs !== false && currentInstance.hasFallthrough) {
			const owner = currentInstance;
			const source = () => resolveFallthroughAttrs(owner);
			if (rawProps && rawProps !== EMPTY_OBJ) (rawProps.$ || (rawProps.$ = [])).push(source);
		...
		let inputScope;
		if (keepAliveCtx && (rawProps && !once || rawSlots && rawSlots.$)) {
		...
		onScopeDispose$1(() => unmountComponent(instance, void 0, isInteropEnabled ? resolveUnmountSuspense(instance.suspense) : instance.suspense), true);
...
function getRootElement(block, visitor) {
	...
	if (isFragment(block) && !(isTeleportEnabled && isTeleportFragment(block))) {
		if (visitor) {
			...
			if (isDynamicFragment(block) && visitor.onDynamicFragment) visitor.onDynamicFragment(block);
	...
	if (isArray(block)) {
		...
		if (!hasComment || single === void 0) return;
...
function applyFallthroughAttrs(block, instance, scope) {
	...
	if (root && !hasSlotFragment) {
		...
		if (Object.keys(fallthroughAttrs).length && (hasSlotFragment || fragments && state.hasNonSingleRoot || isTeleportEnabled && containsTeleportFragment(block) || !accessedAttrs && (block instanceof Text || isArray(block) && block.length))) warnExtraneousAttributes(instance.attrs);
	}
}
	...
	if (isFragment(block) && !(isTeleportEnabled && isTeleportFragment(block))) {
		if (isDynamicFragment(block)) {
			if (block.__vf & 4) {
				...
				(state.fragments || (state.fragments = [])).push(block);
				if (state.innermost && state.innermost.scope) state.parentScope = state.innermost.scope;
		...
		if (!el && state.innermost === block) state.hasNonSingleRoot = isArray(nodes) && nodes.some((child) => !(child instanceof Comment));
		return el;
	}
	if (isArray(block)) {
		let single;
...
function defineVaporComponent(comp, extraOptions) {
	if (isFunction(comp)) return /*@__PURE__*/ (() => extend$1({ name: comp.name }, extraOptions, {
		setup: comp,
		__vapor: true
	}))();
	comp.__vapor = true;
...
function resolveTransitionHooks(block, props, state, instance, postClone) {
	const context = getTransitionHooksContext(block, props, state, instance, postClone);
...
const VaporKeepAlive = /*@__PURE__*/ withKeepAliveEnabled(/* @__PURE__ */ defineVaporComponent({
	...
	setup(props, { slots, expose }) {
		...
		const cacheBlock = (block = keepAliveInstance.block) => {
			...
			const branchKey = isDynamicFragment(block) && block.keyed ? block.current : void 0;
...
function prepareInteropSlotTransition(frag, vnode, isDirectSlotRoot, previous, resumeAfterLeave, delayedLeaveSource) {
	...
	if (!transition && !(isDirectSlotRoot && instance && isVaporTransition(instance.type))) return;
...
function renderVDOMSlot(internals, slotsRef, name, props, parentComponent, options = EMPTY_OBJ) {
	const { fallback, once, slotRoot, sharedFallback, inheritFallback, adoptAnchor } = options;
	...
	frag.insert = (parentNode, anchor, parentSuspense) => {
		...
		if (!isMounted) {
			...
			} else if (isVNode(rendered)) internals.m(rendered, parentNode, anchor, 2, parentComponent, suspense);
			else if (rendered) insert(rendered, parentNode, anchor, suspense);
	...
	function resumeOutIn() {
		queuePostFlushCb(() => {
			...
			if (isVNode(pendingContent)) {
				const nextVNode = prepareInteropSlotTransition(frag, pendingContent, isDirectSlotRoot, void 0, NOOP) || pendingContent;
	...
	function applyVNodeContent(slotContent, slotContentValid) {
		...
		if (prevVNode && !slotContentValid && transition && hasSlotFallback(boundary)) {
			if (mode === "out-in") {
				const placeholder = prepareInteropSlotTransition(frag, slotContent, isDirectSlotRoot, void 0, NOOP) || createCommentVNode();
		...
		const transitionChild = prepareInteropSlotTransition(frag, slotContent, isDirectSlotRoot, prevVNode || void 0, resumeOutIn, delayedLeaveSource);
	...
	*/
	function hydrateContent(slotContent, slotContentValid) {
		...
		if (isVNode(hydratedContent)) {
			const hydrationVNode = prepareInteropSlotTransition(frag, hydratedContent, isDirectSlotRoot, void 0, NOOP) || hydratedContent;
...
function renderVaporSlot(vnode, parentComponent, parentSuspense, contextSlotScopeIds) {
	...
	try {
		...
		const frag = createInteropFragment(EMPTY_BLOCK, null, 128);
		frag.slotScopeIds = getInteropVaporSlotScopeIds(vnode, parentComponent, contextSlotScopeIds);
		const content = new InteropContentState();
		frag.isBlockValid = (componentAsValid) => content.resolved ? isValidBlock(frag.nodes, componentAsValid) : true;
		const slotBoundary = frag.slotBoundary;
...
function createIf(condition, b1, b2, flags = 1) {
	...
	if (flags & 16) {
		...
		const dynamicFragment = new DynamicFragment(256, !!(process.env.NODE_ENV !== "production") ? "if" : void 0, keyed, false, trackSlotBoundary, trackSlotBoundary ? () => {
...
function createKeyedFragment(key, render, trackSlotBoundary = false) {
	...
	const frag = new DynamicFragment(0, !!(process.env.NODE_ENV !== "production") ? "keyed" : void 0, true, true, trackSlotBoundary, trackSlotBoundary ? () => {
...
const createFor = (src, renderItem, getKey, flags = 0) => {
	...
	const renderList = () => {
			...
			if (!oldLength) mountAll(source, newLength);
				...
				for (let q = queuedLength - 1; q >= 0; q--) {
					...
					if (anchorNode === void 0) {
						if (cachedAnchor === void 0) {
							cachedAnchor = parentAnchor;
							if (isTransitionEnabled && frag.$transition) for (let i = e2 - 1; i >= 0; i--) {
								const oldBlock = oldBlocks[i];
					...
					if (block !== void 0) insertForBlock(block, anchorNode);
		...
		if (wasMounted && frag.onUpdated) for (const fn of frag.onUpdated) fn();
		setActiveSub(prevSub);
	};
	...
	const mount = (source, idx, anchor) => {
		...
		const itemRef = shallowRef$1(getItemValue(source, idx));
		const keyRef = needKey ? shallowRef$1(keys ? keys[idx] : idx) : void 0;
		const indexRef = needIndex ? shallowRef$1(keys ? idx : void 0) : void 0;
		let nodes;
		const scope = new EffectScope$1(true);
...
function createDynamicComponent(getter, rawProps, rawSlots, flags = 0) {
	...
	if (!isHydrating$1) resetInsertionState();
	const hydrationCursor = isHydrating$1 ? captureHydrationCursor() : null;
	const frag = new DynamicFragment(0, !!(process.env.NODE_ENV !== "production") ? "dynamic-component" : void 0, false, true, slotRoot, slotRoot ? () => {
	...
	const renderFn = () => {
		...
		const resolved = isBlock(value) || isInteropEnabled && appContext.vdom && isVNode(value) ? value : withScopeOwner(scopeOwner, () => resolveDynamicComponent(value));
		...
		frag.update(() => {
			...
			if (isInteropEnabled && appContext.vdom && isVNode(value)) {
				...
				const owner = isSingleRoot && isVaporComponent(currentInstance) && currentInstance.hasFallthrough && currentInstance.type.inheritAttrs !== false ? currentInstance : void 0;
				const frag = appContext.vdom.mountVNode(value, currentInstance, owner && (() => resolveFallthroughAttrs(owner)));
				if (isHydrating$1) {
			...
			return createComponentWithFallback(resolved, rawProps, normalizedRawSlots, isSingleRoot, once, appContext);
		}, value);
	};
	if (once) renderFn();
...
function withVaporDirectives(node, dirs) {
	...
	function trackRootUpdates(block) {
		if (isVaporComponent(block)) {
			...
			return isAsyncWrapper(block) && isFragment(innerBlock) && innerBlock.nodes === EMPTY_BLOCK;
		}
		if (isArray(block)) {
...
function ensureForHydrationAnchorResolver() {
	...
	setForHydrationAnchorResolver((hydrationStart, anchorNode) => {
		...
		const parentAnchor = claimAnchor(!!(process.env.NODE_ENV !== "production") ? /* @__PURE__ */ createComment("for") : /* @__PURE__ */ createTextNode());
...
function collectTransitionBlocks(block, children, onFragment, onUpdateOwner, collectOnly = false) {
	if (block instanceof Node) {
		if (block instanceof Element) children.push(block);
	} else if (isVaporComponent(block)) {
		...
		const start = children.length;
		collectTransitionBlocks(block.block, children, onFragment, isRootSlot ? onUpdateOwner : void 0, collectOnly);
		if (!collectOnly) {
	...
	} else if (isArray(block)) for (let i = 0; i < block.length; i++) collectTransitionBlocks(block[i], children, onFragment, onUpdateOwner, collectOnly);
		...
		if (isInteropEnabled && block.hasVDOMContent && block.hasVDOMContent()) children.push(block);
			...
			const start = children.length;
			collectTransitionBlocks(block.nodes, children, onFragment, onUpdateOwner, collectOnly);
			if (collectOnly) {} else if (isItem) {
	...
	for (let i = 0; i < children.length; i++) {
		...
		if (isValidTransitionBlock(child)) {
			if (child.$key != null) {
				...
				child.$transition = resolveTransitionHooks(child, props, state, instance);
